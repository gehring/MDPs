import numpy as np
from pymdp.MarkovChain import MarkovChainWithReward
...
class MDP(object):
    ...
    def generateMC(self, pi):
        prob = pi[:,0]
        P = self.Ps[0] * prob[:,None]
        for i in xrange(1, len(self.Ps)):
            prob = pi[:,i]
            P += self.Ps[i] * prob[:,None]
            
        return MarkovChainWithReward(P, self.R)
